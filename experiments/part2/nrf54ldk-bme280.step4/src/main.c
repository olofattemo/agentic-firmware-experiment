...
int main(void)
{
	...
	while (1) {
		...
		if (sensor_sample_fetch(dev) < 0) {
			...
			int t_err = sensor_channel_get(dev, SENSOR_CHAN_AMBIENT_TEMP, &temp);
			int h_err = sensor_channel_get(dev, SENSOR_CHAN_HUMIDITY, &hum);

			if (t_err < 0 || h_err < 0) {
				printf("Failed to get sensor data (T: %d, H: %d)\n", t_err, h_err);
				success = false;
			} else {
